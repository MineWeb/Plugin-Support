...
class SupportSecurityComponent extends CakeObject
{
    /*
    ...
    */
    ...
    function xssProtection($content)
    {
        return $this->htmLawed($content, ['safe' => 1]);
    }
    ...
    function htmLawed($t, $C = 1, $S = array())
    {
        ...
        $C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or $this->hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or $this->hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
    ...
    function hl_tag($t)
    {
        ...
        if ($C['make_tag_strict'] && isset($eD[$e])) {
            $trt = $this->hl_tag2($e, $a, $C['make_tag_strict']);
            if (!$e) {
                return (($C['keep_bad'] % 2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
            }
        }
        ...
        foreach ($aA as $k => $v) {
            if (((isset($d['*']) ? isset($d[$k]) : !isset($d[$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or (isset($aNO[$k]) && !isset($d['on*'])) or (isset($aNA[$k]) && !isset($d['aria*'])) or (!isset($d['data*']) && preg_match('`data-((?!xml)[^:]+$)`', $k))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
                ...
                if ($k == 'style' && !$C['style_pass']) {
                    ...
                    if ($k == 'srcset') {
                        ...
                        foreach (explode(',', $v) as $k1 => $v1) {
                            ...
                            $v1 = trim($v1[0]);
                            if (isset($v1[0])) {
                                $v2 .= $this->hl_prot($v1, $k) . (empty($k1) ? '' : ' ' . $k1) . ', ';
                            }
                        }
                        $v = trim($v2, ', ');
                    }
                ...
                if (isset($rl[$k]) && is_array($rl[$k]) && ($v = $this->hl_attrval($k, $v, $rl[$k])) === 0) {
                    continue;
                }
                $a[$k] = str_replace('"', '&quot;', $v);
            }
